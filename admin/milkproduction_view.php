...
if(isset($_GET['breed']))
{
	$reslt = mysqli_query($conn,"SELECT * from milkproduction where (breed ='".$_GET['breed']."' OR location like '%".$_GET['location']."%' )");
...
<?php if($count == 0)
				{ ?>
				<div class="col-group">
					<h3> No results Found oF <?php echo $_GET['breed']; ?></h3>
				</div>
				 <?php
				 } else
				 {?>
	...
	<div class="form-group row">
    <label class="col-sm-3 text-right control-label col-form-label" for="psw"><b>breed :</b>
