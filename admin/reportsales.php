...
$conn = mysqli_connect($servername, $username, $password, $dbname);
	...
	if(isset($_GET['fdate']))
	{
		$fdate= $_GET['fdate'];
		$tdate= $_GET['tdate'];
	$reslt = mysqli_query($conn,"select * from salesfeed  where (date BETWEEN '".$fdate."' AND '".$tdate."') and location='".$_GET['location']."'  ");
	$count=mysqli_num_rows($reslt);
	$reslt1 = mysqli_query($conn,"select * from salescattle where (date BETWEEN '".$fdate."' AND '".$tdate."') and location='".$_GET['location']."'  ");
	$count1=mysqli_num_rows($reslt1);
	$reslt2 = mysqli_query($conn,"select * from salesmilkproducts where (date BETWEEN '".$fdate."' AND '".$tdate."') and location='".$_GET['location']."'  ");
...
if(mysqli_num_rows($reslt)>0)
		...
		?>
		<h3>Milk products sales reports</h3>
		<table border=1>
			<tr>
				<th>Id</th>
				<th>Product Id</th>
