...
if(isset($_POST['submit']))
	{
		...
		$reslt = mysqli_query($conn,"SELECT * FROM store WHERE username='".$username."' ");
		$count=mysqli_num_rows($reslt);
		if($count==0)
		{
			$reslts = mysqli_query($conn,"INSERT INTO `store`(fname,lname,`username`, `email`, `password`) VALUES ('".$fname."','".$lname."','".$username."','".$email."','".$password."')");
			echo "Storekeeper Registered successfully";
		}
		else
		{
