...
$conn = mysqli_connect($servername, $username, $password, $dbname);
	if(isset($_GET['fdate']))
	{
		$fdate= $_GET['fdate'];
		$tdate= $_GET['tdate'];
$reslt = mysqli_query($conn,"select * from stockfeeding  where (date BETWEEN '".$fdate."' AND '".$tdate."') and location='".$_GET['location']."'  ");
	$count=mysqli_num_rows($reslt);
	$reslt1 = mysqli_query($conn,"select * from stockvaccine  where (date BETWEEN '".$fdate."' AND '".$tdate."') and location='".$_GET['location']."'  ");
	$count1=mysqli_num_rows($reslt1);
	$reslt2 = mysqli_query($conn,"select * from stockmilk  where (date BETWEEN '".$fdate."' AND '".$tdate."') and location='".$_GET['location']."'  ");
	$count2=mysqli_num_rows($reslt2);
	}
include("index.php"); 
?>
...
if(mysqli_num_rows($reslt)>0)
		...
		{
			echo "<h1>No records for cattle sales<h1>";
		}
		if(mysqli_num_rows($reslt2)>0)
		{
		
		?>
		<h3>Milk stock reports</h3>
		<table border=1>
			<tr>
				<th>Id</th>
				<th>Milk Product Name</th>
				<th>Location</th>
				<th>date</th>
				<th>quantity</th>
				<th>Description</th>
		...
		{
			echo "<h1>No records of milk products sales</h1>";
		}
}
		?>  
</div>
